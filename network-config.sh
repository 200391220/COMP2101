...
# Setting variables
hostname=$(hostname)
interfacename=$(ip a |awk '/: e/{gsub(/:/,"");print $2}') # ip -a means all ip address and use regex expression to replace ip addresses with "" and just get iterfacename
lanaddress=$(ip a s $interfacename|awk '/inet /{gsub(/\/.*/,"");print $2}') # we get lan address by using regex to just get lanaddress
lanhostname=$(getent hosts $lanaddress| awk '{print $2}') # We get lanhostname from administrative db name "hosts" by passing lanaddress and extracting 2nd col
externalip=$(curl -s icanhazip.com) # curl is use to download something from internet and -s is silent mode and "icanhazip.com" is website which shows our public ip address in text format so using curl we download file from website which is text
externalName=$(getent hosts $externalip | awk '{print $2}') # We check host name for our ip address on hosts database
routeraddress=$(ip r | awk '/via /{gsub(/\/.*/,"");print $3}') # from ip resolve command extract ip address after "/via"
routerhostname=$(getent hosts $routeraddress | awk '{print $2}') # Check router hostname on 'hosts' db by passing router address
